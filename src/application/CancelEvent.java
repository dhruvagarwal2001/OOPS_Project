...
public class CancelEvent {
	public static void display(Audi a,Student s,Event e)
	{
		...
		label.setText(e.getName()+":"+e.getDesc()+"\nTime :"+e.getStartTime()+" - "+e.getEndTime()+"\nCost :"+e.getCost()+"\n'*'-Booked Seats\n'_'-Empty Seats");
		...
		for(int i=0;i<20;i++)
		{
			...
			for(int j=0;j<10;j++)
			{
				...
				{
					if(!e.getAvailability(i/2, j) && e.getStudentForBookedSeat(i/2, j).equals(s))
					{
		...
		Label EventId=new Label("How Many Tickets do you want to cancel:");
		...
		Label Seatnum=new Label("Enter the Seat Numbers seperated by ',' that you want to cancel");
		GridPane.setConstraints(EventId,0,row);
		row++;
		TextField seatnum=new TextField();
		seatnum.setPromptText("1,2");
		GridPane.setConstraints(seatnum, 1, row);
		row++;
...
int n=ch.getValue();
			...
			String yo=seatnum.getText();
			String ar[]=yo.split(",");
			int arr[]=new int[n];
			if(n<=0 || ar.length>n || ar.length<n || ar.length==0)
			{
				AlertBox.display("Alert", "Enter correct number of seats", "Close");
				CancelEvent.display(a, s, e);
				return;
			}
			try {
			for(int i=0;i<n;i++)
			...
			if(e.cancelTicks(x,y,n,s)==0)
			{
				AlertBox.display("Alert", "Choose Seats that are booked only", "Close");	
			}
			else
				{
		...
		layout.getChildren().addAll(Seatnum,seatnum,logout,back,cancel);
